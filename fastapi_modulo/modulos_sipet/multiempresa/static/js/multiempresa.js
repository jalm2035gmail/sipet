(function () {
  ...
  async function loadKpis() {
    try {
      const d = await api('GET', '/api/multiempresa/kpis');
  ...
  async function loadEmpresas() {
    ...
    const q = $('filter-nombre-empresa').value.trim();
    try {
      const params = new URLSearchParams();
      if (estado) params.set('estado', estado);
      if (q) params.set('q', q);
      const url = '/api/multiempresa/empresas' + (params.toString() ? '?' + params.toString() : '');
      const result = await api('GET', url);
      _empresas = result.items;
      renderEmpresas();
  ...
  $('filter-nombre-empresa').addEventListener('input', loadEmpresas);
