...
def _recent_login_stats(username: str, ip: str, created_at: datetime, lookback_minutes: int = 60) -> dict[str, int]:
    db = SessionLocal()
    try:
...
def _heuristic_training_label(feature_row: dict[str, Any]) -> str:
    score = _fallback_score(feature_row)
...
def sklearn_available() -> bool:
    return all(item is not None for item in (joblib, pd, ColumnTransformer, OneHotEncoder, Pipeline, RandomForestClassifier))


def train_access_risk_model(hours: int = 24 * 30, output_path: str = MODEL_PATH) -> dict[str, Any]:
    dataset = build_training_dataset(hours)
    if not dataset:
        return {"status": "empty", "model_path": "", "samples": 0}
    if not sklearn_available():
        return {"status": "fallback", "model_path": "", "samples": len(dataset)}
    ...
    model = Pipeline(
        steps=[
            ...
            ("classifier", RandomForestClassifier(n_estimators=80, random_state=42, class_weight="balanced")),
        ]
    )
    ...
    return {"status": "trained", "model_path": output_path, "samples": len(dataset)}
    ...
    if model_payload and pd is not None:
        ...
        try:
            ...
            if hasattr(pipeline, "predict_proba"):
                ...
                raw_probabilities = list(pipeline.predict_proba(frame)[0])
                probabilities = {str(name): float(value) for name, value in zip(classes, raw_probabilities)}
            risk_score = probabilities.get("sospechoso")
            if risk_score is None:
                risk_score = 1.0 if predicted_label == "sospechoso" else 0.55 if predicted_label == "inusual" else 0.1
            return {
