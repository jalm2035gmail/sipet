...
export const createContactosModule = ({ root, state, refreshResumen, refreshCatalogos }) => {
  ...
  const openDetailModal = async (row) => {
    openModal(`<p class="crm-status">Cargando detalle…</p>`);
    const [seguimiento, actividadesResp, notasResp] = await Promise.all([
      apiGet(`/api/crm/seguimiento?contacto_id=${row.id}`).catch(() => []),
      apiGet(`/api/crm/actividades?contacto_id=${row.id}`).catch(() => ({})),
      apiGet(`/api/crm/notas?contacto_id=${row.id}`).catch(() => []),
    ]);
