...
def list_campanias(tenant_id: str, estado: Optional[str] = None) -> List[Dict[str, Any]]:
    db = get_db()
    try:
        query = db.query(CrmCampania).filter(CrmCampania.tenant_id == tenant_id)
        if estado:
            query = query.filter(CrmCampania.estado == estado)
        return [campania_to_dict(row) for row in query.order_by(CrmCampania.creado_en.desc()).all()]
    finally:
        db.close()
