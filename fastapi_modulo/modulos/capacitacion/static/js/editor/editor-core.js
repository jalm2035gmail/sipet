...
var editorBootstrapError = !root
  ? null
  : (!presId
      ? 'Presentación inválida.'
      : (!window.grapesjs ? 'GrapesJS no está disponible.' : null));
...
var sidepanelEl = el('ped-sidepanel');
...
var railButtons = Array.prototype.slice.call(document.querySelectorAll('.ped-rail-btn'));
...
var allPanelEls = (function () {
  var ids = [
    'ped-text-panel',
    'ped-resource-panel',
    'ped-interactive-panel',
    'ped-questions-panel',
    'ped-widgets-panel',
    'ped-insert-panel',
    'ped-style-panel',
    'ped-background-panel',
    'ped-pages-panel'
  ];
  return ids.map(function (id) { return el(id); }).filter(Boolean);
})();
...
function setDefaultSidepanelCopy() {
  ...
  if (sidepanelCopy) {
    sidepanelCopy.textContent = 'Bloques, estilos, capas y propiedades de cada slide.';
  }
}
    ...
    if (editor.Canvas.getBody) {
      ...
      if (canvasBody) {
        ...
        if (slide.bg_image_url) {
          ...
          canvasBody.style.backgroundSize = '';
          canvasBody.style.backgroundPosition = '';
          canvasBody.style.backgroundRepeat = '';
        }
      }
    }
  } catch (error) {}
...
function renderAuditTrail(items) {
  ...
  auditList.innerHTML = items.slice(0, 8).map(function (item) {
    ...
    var date = item.creado_en
      ? String(item.creado_en).replace('T', ' ').slice(0, 16)
      : 'Sin fecha';

    return (
      '<article class="ped-audit-item">' +
...
function loadAuditTrail() {
  ...
  return apiJson('/api/capacitacion/auditoria/presentacion/' + presId)
    .then(function (res) {
      var items = res && res.ok && res.data && Array.isArray(res.data.items)
        ? res.data.items
        : [];
...
function bindBackgroundInputs() {
  ...
  inputSlideBgColor.addEventListener('change', function () {
    if (typeof applyBackgroundColor === 'function') {
...
function hideAllSecondaryPanels() {
  allPanelEls.forEach(function (panel) {
    panel.classList.remove('is-active');
  });
}

function closeSecondaryPanel() {
  if (workspace) workspace.classList.add('is-sidepanel-collapsed');
...
function openSecondaryPanel(buttonId) {
  var panelId = panelMap[buttonId];
  var targetPanel = panelId ? el(panelId) : null;
  if (!targetPanel) return;

  if (workspace) workspace.classList.remove('is-sidepanel-collapsed');
...
function setSidepanelCollapsed(collapsed) {
  ...
  var activeBtn = null;
  railButtons.forEach(function (btn) {
    if (!activeBtn && btn.classList.contains('is-active')) activeBtn = btn;
  });

  if (activeBtn) {
    openSecondaryPanel(activeBtn.id);
    return;
  }

  if (railButtons.length) {
    openSecondaryPanel(railButtons[0].id);
  }
}
...
function fitSlideSurface() {
  if (!editor || !editor.Canvas || !stageWrap || !root) return;

  try {
    var viewport = getCanvasViewport();
    var stage = root.querySelector('.ped-stage');
    var referenceNode = stage || stageWrap;
    var rect = referenceNode.getBoundingClientRect();
...
window.pedCore = window.pedCore || {};

window.pedCore.el = el;
window.pedCore.esc = esc;
window.pedCore.toast = toast;
window.pedCore.apiJson = apiJson;
window.pedCore.currentSlide = currentSlide;
window.pedCore.paintEditorBackground = paintEditorBackground;
window.pedCore.loadAuditTrail = loadAuditTrail;
window.pedCore.renderAuditTrail = renderAuditTrail;
window.pedCore.bindBackgroundInputs = bindBackgroundInputs;
window.pedCore.setDefaultSidepanelCopy = setDefaultSidepanelCopy;
window.pedCore.openSecondaryPanel = openSecondaryPanel;
window.pedCore.closeSecondaryPanel = closeSecondaryPanel;
window.pedCore.setSidepanelCollapsed = setSidepanelCollapsed;
window.pedCore.refreshEditorCanvas = refreshEditorCanvas;
window.pedCore.fitSlideSurface = fitSlideSurface;
window.pedCore.injectCanvasStyles = injectCanvasStyles;
...
window.paintEditorBackground = paintEditorBackground;
window.loadAuditTrail = loadAuditTrail;
window.renderAuditTrail = renderAuditTrail;
window.bindBackgroundInputs = bindBackgroundInputs;
window.setDefaultSidepanelCopy = setDefaultSidepanelCopy;
window.openSecondaryPanel = openSecondaryPanel;
window.closeSecondaryPanel = closeSecondaryPanel;
window.setSidepanelCollapsed = setSidepanelCollapsed;
window.refreshEditorCanvas = refreshEditorCanvas;
window.fitSlideSurface = fitSlideSurface;
window.injectCanvasStyles = injectCanvasStyles;
