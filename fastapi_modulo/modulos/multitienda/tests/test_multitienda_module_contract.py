...
def test_multitienda_manifest_declares_runtime_dependencies() -> None:
    ...
    integrations = {item.get("module_key") for item in (MANIFEST.get("integration_points") or [])}
...
def test_multitienda_main_is_thin_bootstrap() -> None:
    ...
    assert "reservaciones_html()" not in entrypoint_source
    assert "institucion_financiera_html()" not in entrypoint_source
    assert "repartidores_html()" not in entrypoint_source
    assert "/multitienda/api/referidos" not in entrypoint_source
    assert "/multitienda/api/reservaciones" not in entrypoint_source
    assert "app.include_router(referrals_router" not in controller_source
    assert "app.include_router(reservations_router" not in controller_source
