...
from fastapi_modulo.modulos.web.modelos.db_models import WebLoginAttempt, WebSecurityEvent, WebUserSession
...
def users_with_mfa_disabled(limit: int = 100) -> list[dict[str, Any]]:
    ...
    try:
        rows = db.query(core.Usuario).limit(max(1, int(limit))).all()
        disabled: list[dict[str, Any]] = []
        for row in rows:
            has_totp = bool(getattr(row, "totp_enabled", False) and getattr(row, "totp_secret", ""))
            has_passkey = bool(getattr(row, "backendauthn_credential_id", "") and getattr(row, "backendauthn_public_key", ""))
            if has_totp or has_passkey:
                continue
            disabled.append(
                {
                    "user_id": int(getattr(row, "id", 0) or 0),
                    "username": str(getattr(row, "usuario", "") or ""),
                    "role_id": int(getattr(row, "rol_id", 0) or 0),
                }
            )
        return disabled
    except Exception:
