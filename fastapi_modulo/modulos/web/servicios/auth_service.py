...
from fastapi_modulo.modulos.web.servicios.access_service import normalize_role_name, sensitive_lookup_hash
...
from fastapi_modulo.modulos.web.servicios.security_policy_service import max_concurrent_sessions, should_revoke_other_sessions
...
def find_user_by_login(db, login_value: str):
    ...
    user = db.query(core.Usuario).filter(core.Usuario.usuario_hash == login_hash).first()
    if not user:
        user = db.query(core.Usuario).filter(core.Usuario.correo_hash == login_hash).first()
    if not user:
        user = db.query(core.Usuario).filter(core.func.lower(core.Usuario.usuario) == normalized_login).first()
    if not user:
        user = db.query(core.Usuario).filter(core.func.lower(core.Usuario.correo) == normalized_login).first()
    return user
...
def find_user_by_id(db, user_id: int):
    ...
    return db.query(core.Usuario).filter(core.Usuario.id == int(user_id)).first()


def get_session_local():
    from fastapi_modulo import main as core

    return core.SessionLocal
...
def resolve_user_role_name(db, user) -> str:
    ...
    role_name = "usuario"
    if user.rol_id:
        role = db.query(core.Rol).filter(core.Rol.id == user.rol_id).first()
...
def get_user_totp_secret(user, role_name: str) -> str:
    from fastapi_modulo.modulos.web.servicios.mfa_service import get_user_totp_secret as get_totp_secret

    return get_totp_secret(user, role_name)

def verify_totp_code(secret: str, code: str) -> bool:
    from fastapi_modulo.modulos.web.servicios.mfa_service import verify_totp_code as verify_code
...
def passkey_rp_id(request: Request) -> str:
    from fastapi_modulo.modulos.web.servicios.passkey_service import passkey_rp_id as resolve_rp_id

    return resolve_rp_id(request)


def passkey_origin(request: Request) -> str:
    from fastapi_modulo.modulos.web.servicios.passkey_service import passkey_origin as resolve_origin

    return resolve_origin(request)


def build_passkey_token(action: str, user_id: int, challenge: str, rp_id: str, origin: str) -> str:
    from fastapi_modulo.modulos.web.servicios.passkey_service import build_passkey_token as build_token
...
def read_passkey_token(token: str, expected_action: str) -> Optional[dict[str, Any]]:
    from fastapi_modulo.modulos.web.servicios.passkey_service import read_passkey_token as read_token

    return read_token(token, expected_action)


def build_mfa_gate_token(user_id: int) -> str:
    from fastapi_modulo.modulos.web.servicios.passkey_service import build_mfa_gate_token as build_token

    return build_token(user_id)


def parse_client_data(client_data_b64: str) -> Optional[dict[str, Any]]:
    from fastapi_modulo.modulos.web.servicios.passkey_service import parse_client_data as parse_payload
...
def build_passkey_registration(request: Request, user, username: str) -> tuple[dict[str, Any], str]:
    from fastapi_modulo.modulos.web.servicios.passkey_service import build_passkey_registration as build_registration
...
def build_passkey_authentication(request: Request, user) -> tuple[dict[str, Any], str]:
    from fastapi_modulo.modulos.web.servicios.passkey_service import build_passkey_authentication as build_authentication

    return build_authentication(request, user)


def set_passkey_cookie(response: Response, cookie_name: str, token: str) -> None:
    from fastapi_modulo.modulos.web.servicios.passkey_service import set_passkey_cookie as set_cookie
