...
from fastapi_modulo.modulos.aplicaciones.repositorios.app_repository import list_catalog_modules
from fastapi_modulo.modulos.aplicaciones.repositorios.package_repository import (
    PROJECT_ROOT,
...
from fastapi_modulo.modulos.aplicaciones.servicios.audit_service import get_protocol_audit_map
from fastapi_modulo.modulos.aplicaciones.servicios.image_branding_service import get_module_catalog_image_url
from fastapi_modulo.modulos.aplicaciones.servicios.redis_service import cache_catalog, get_cached_catalog


def decorate_modules_payload(items: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    if items is None:
        cached = get_cached_catalog()
        if cached is not None:
            return cached
    payload = list_catalog_modules() if items is None else items
    protocol_map = get_protocol_audit_map()
    persisted_state = list_registry_state()
    for item in payload:
