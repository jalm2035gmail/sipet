...
def _get_user_password_hash(username: str) -> str:
    ...
    from fastapi_modulo.modulos.web.servicios.access_service import sensitive_lookup_hash
    ...
    try:
        user = db.query(core.Usuario).filter(core.Usuario.usuario_hash == sensitive_lookup_hash(normalized)).first()
        if not user:
            return ""
...
def _verify_password(password: str, stored_hash: str) -> bool:
    from fastapi_modulo.main import verify_password as verify_main_password
    ...
    try:
        ...
        return False
    return verify_main_password(password, stored)
