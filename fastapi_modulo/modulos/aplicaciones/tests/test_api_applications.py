...
from fastapi_modulo.modulos.aplicaciones.controladores.aplicaciones import router
from fastapi_modulo.modulos.aplicaciones.controladores import api_packages, api_protocol, api_state
...
def test_api_list_modules_returns_catalog(monkeypatch) -> None:
    ...
    monkeypatch.setattr(api_state, "decorate_modules_payload", lambda items=None: [_module_item()])
...
def test_api_update_module_state(monkeypatch) -> None:
    monkeypatch.setattr(api_state, "require_applications_permission", lambda request, permission: None)
    monkeypatch.setattr(api_state, "request_actor_context", lambda request: {"user_id": "tester", "tenant_id": "default", "ip": "127.0.0.1"})
...
def test_api_sync_protocol_inline(monkeypatch) -> None:
    ...
    monkeypatch.setattr(api_protocol, "request_actor_context", lambda request: {"user_id": "tester", "ip": "127.0.0.1"})
...
def test_api_upload_valid_zip(monkeypatch) -> None:
    ...
    monkeypatch.setattr(api_packages, "request_actor_context", lambda request: {"user_id": "tester", "tenant_id": "default", "ip": "127.0.0.1"})
