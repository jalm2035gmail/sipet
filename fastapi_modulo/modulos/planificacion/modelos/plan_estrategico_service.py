...
def _ensure_strategic_identity_table(db) -> None:
    ...
    for row in legacy_rows:
        try:
            ...
            """
            if _is_postgres(db):
                insert_sql += " ON CONFLICT (tenant_id, bloque) DO NOTHING"
            else:
...
def _ensure_axis_kpi_table(db) -> None:
    ...
    try:
        col_names = _column_names(db, "strategic_axis_kpis")
...
def _ensure_objective_kpi_table(db) -> None:
    ...
    try:
        col_names = _column_names(db, "strategic_objective_kpis")
...
def _ensure_objective_milestone_table(db) -> None:
    ...
    try:
        col_names = _column_names(db, "strategic_objective_milestones")
