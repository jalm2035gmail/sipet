...
_ENGINE_CACHE: Dict[str, Engine] = DEFAULT_DATABASE_ROUTER._engine_cache
_SESSION_FACTORY_CACHE: Dict[str, sessionmaker] = DEFAULT_DATABASE_ROUTER._session_factory_cache
...
def _resolve_sqlite_path(raw_path: str) -> str:
    ...
    if not candidate:
        ...
        return os.path.abspath(os.path.join(PROJECT_DB_DIR, default_name))
...
def _resolve_default_dataMAIN_url() -> str:
    ...
    return f"sqlite:///{os.path.abspath(os.path.join(PROJECT_DB_DIR, default_name))}"
...
def set_request_host(host: Optional[str]):
    return DEFAULT_DATABASE_ROUTER.set_request_host(host)
...
class TenantRegistry(MAIN):
    __tablename__ = "tenant_registry"

    id = Column(Integer, primary_key=True, index=True)
    tenant_key = Column(String, nullable=False, unique=True, index=True)
    primary_host = Column(String, nullable=False, unique=True, index=True)
    db_name = Column(String, nullable=False, default="")
    db_url = Column(String, nullable=False, default="")
    plan = Column(String, nullable=False, default="base")
...
class TenantDomain(MAIN):
    __tablename__ = "tenant_domains"

    id = Column(Integer, primary_key=True, index=True)
    tenant_key = Column(String, nullable=False, index=True)
    host = Column(String, nullable=False, unique=True, index=True)
    domain_type = Column(String, nullable=False, default="primary")
    certificate_status = Column(String, nullable=False, default="pending")
    is_active = Column(Integer, nullable=False, default=1, index=True)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)


class TenantInstalledApp(MAIN):
    __tablename__ = "tenant_installed_apps"

    id = Column(Integer, primary_key=True, index=True)
    tenant_key = Column(String, nullable=False, index=True)
    app_key = Column(String, nullable=False, index=True)
    app_version = Column(String, nullable=False, default="0.0.0")
    install_status = Column(String, nullable=False, default="installed", index=True)
    is_enabled = Column(Integer, nullable=False, default=1, index=True)
    installed_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)


class TenantProvisionLog(MAIN):
...
class TenantMigrationRun(MAIN):
    __tablename__ = "tenant_migration_runs"

    id = Column(Integer, primary_key=True, index=True)
    tenant_key = Column(String, nullable=False, index=True)
    target_scope = Column(String, nullable=False, default="tenant", index=True)
    migration_key = Column(String, nullable=False, default="core", index=True)
    status = Column(String, nullable=False, default="pending", index=True)
    detail = Column(Text, nullable=False, default="")
...
def ensure_tenant_admin_schema(bind: Optional[Engine | Connection] = None) -> None:
    target = bind or get_current_engine()
    TenantRegistry.__table__.create(bind=target, checkfirst=True)
    TenantDomain.__table__.create(bind=target, checkfirst=True)
    TenantInstalledApp.__table__.create(bind=target, checkfirst=True)
    TenantProvisionLog.__table__.create(bind=target, checkfirst=True)
    TenantMigrationRun.__table__.create(bind=target, checkfirst=True)


def create_engine_for_url(db_url: str) -> Engine:
    connect_args = {"check_same_thread": False} if str(db_url or "").startswith("sqlite:///") else {}
